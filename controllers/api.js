 ...
 * upload file to S3
 */
exports.uploadFileToS3 = (req, res, next) => {
  upload(req, res, function (err) {
    if (err) {
      // An error occurred while uploading
      console.log(err);
    ...
    fs.rename('uploads/' + file.filename, 'uploads/' + newFileName, function (err) {
      ...
      var accessKeyId = config.Access_key;
      var secretAccessKeyId = config.Secret_key;
      var bucketName = config.bucketName;

      fs.readFile(path, function (error, file_buffer) {
        if (error) {
          console.log("s3 bucket prblm11111------err", error)
          //var errResp = util.createErrorResponse();
        ...
        s3bucket.upload(params, function (err, data) {
          console.log("-----s3 Bucket222222----err---", err);
          console.log(data);
          if (err) {
            console.log(err);
          }
 ...
 * Get files from S3
 */
exports.getS3Files = (req, res, next) => {
  var accessKeyId = config.Access_key;
  var secretAccessKeyId = config.Secret_key;
  var bucketName = config.bucketName;
